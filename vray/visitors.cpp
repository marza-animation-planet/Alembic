         ...
         if (refNode)
         {
            ...
            if (refMesh || refSubd)
            {
               ...
               if (refGeom)
               {
                  ...
                  if (!geoup.readBaseMesh(node, refGeom, computeNormals))
                  {
                  ...
                  else
                  {
                     ...
                     {
                        geoup.setMeshSmoothNormals(*refSubd, refGeom);
                     }
...
float* UpdateGeometry::computeMeshSmoothNormals(AbcVRayGeom *geom,
                                                const float *P0,
                                                const float *P1,
                                                float blend)
{
   ...
   size_t bytesize = 3 * geom->numPoints * sizeof(float);
   ...
   for (unsigned int t=0, v=0; t<geom->numTriangles; ++t, v+=3)
   {
      unsigned int pi[3] = { 3 * geom->toPointIndex[v  ],
                             3 * geom->toPointIndex[v+1],
                             3 * geom->toPointIndex[v+2] };
   ...
   for (unsigned int p=0, off=0; p<geom->numPoints; ++p, off+=3)
   {
...
void UpdateGeometry::readMeshNormals(AlembicMesh &node, AbcVRayGeom *geom)
{
   ...
   if (!Nparam.valid() || 
       (Nparam.getScope() != Alembic::AbcGeom::kVaryingScope &&
        Nparam.getScope() != Alembic::AbcGeom::kFacevaryingScope))
   {
      setMeshSmoothNormals(node, geom);
      return;
   }
   else
   {
   ...
   if (instGeom && !ignore)
   {
      ...
      if (geom)
      {
         if (!geom->updatedFrameGeometry)
         {
            ...
            geom->invalidFrame = !readBaseMesh(node, geom, attrs, computeNormals);
   ...
   if (instGeom && !ignore)
   {
      ...
      if (geom)
      {
         if (!geom->updatedFrameGeometry)
         {
            ...
            geom->invalidFrame = !readBaseMesh(node, geom, attrs, true);
