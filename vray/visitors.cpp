...
CollectWorldMatrices::CollectWorldMatrices(AlembicGeometrySource *geosrc, double time)
   : BaseVisitor(geosrc)
   , mTime(time)
{
}
...
AlembicNode::VisitReturn CollectWorldMatrices::enter(AlembicXform &node, AlembicNode *)
{
   if (!node.isLocator() && !mGeoSrc->params()->ignoreTransforms)
   {
      ...
      Alembic::Abc::M44d localMatrix = schema.getValue(Alembic::Abc::ISampleSelector(mTime)).getMatrix();
...
bool UpdateGeometry::readMeshNormals(AlembicMesh &node,
                                     ...
                                     size_t ntimes, double *times, double *frames)
{
   ...
   else
   {
      ...
      if (ntimes == 0)
      {
         ...
         ntimes = (singleSample ? 1 : mGeoSrc->numTimeSamples());
         times = (singleSample ? &renderTime : mGeoSrc->sampleTimes());
         frames = (singleSample ? &renderFrame : mGeoSrc->sampleFrames());
      }
      ...
      if (Nsamples.size() == 1 || varyingTopology)
      {
         ...
         Nsamples.getSamples(times[0], Nsamp0, Nsamp1, Nb);
         ...
         VR::Table<VR::Vector> &nl = info->normals->at(frames[0]);
         nl.setCount(N->size(), true);

         // VR::Table<int> &il = info->faceNormals->at(renderFrame);
         VR::Table<int> &il = info->faceNormals->at(frames[0]);
   ...
   if (instInfo && !ignore)
   {
      ...
      if (info)
      {
         if (!info->updatedFrameGeometry)
         {
            ...
            info->invalidFrame = !readBaseMesh(node, info, attrs, computeNormals, refInfo);
            ...
            else
            {
               ...
               if (refInfo)
               {
                  ...
                  if (useAttrs && hasReferenceNormals(refInfo, &attrs, &nit))
                  {
                     if (mGeoSrc->params()->verbose)
                     {
                     ...
                     setMeshNormals(node, refInfo, &(nit->second));
                  }
                  else
                  {
                     ...
                     else
                     {
                        ...
                        double refFrame = mGeoSrc->renderFrame();
                        double refTime = 0.0;

                        if (!useAttrs)
                        {
                           refTime = mGeoSrc->referenceTime();
                        }
                        else
                        {
                           refTime = schema.getTimeSampling()->getSampleTime(0);
                        }

                        if (!readMeshNormals(node, refInfo, 1, &refTime, &refFrame))
                        {
   ...
   if (instInfo && !ignore)
   {
      ...
      if (info)
      {
         if (!info->updatedFrameGeometry)
         {
            ...
            info->invalidFrame = !readBaseMesh(node, info, attrs, true, refInfo);
