...
if withArnold:
   prjs.append({"name": "%sAlembicArnoldProcedural" % nameprefix,
                ...
                "incdirs": incdirs + ["arnold/Procedural"] + regex_inc,
                "libdirs": libdirs,
                "libs": alembic_libs + ilmbase_libs + hdf5_libs + libs,
...
if excons.GetArgument("with-maya", default=None) is not None:
   ...
   prjs.extend([{"name": "%sAbcImport" % nameprefix,
                 ...
                 "defs": plugin_defs + regex_def + (["ABCIMPORT_VERSION=\"\\\"%s\\\"\"" % impver] if impver else []),
                 "incdirs": incdirs + ["maya/AbcImport"] + regex_inc,
                 "libdirs": libdirs,
                 "libs": alembic_libs + ilmbase_libs + hdf5_libs + libs,
