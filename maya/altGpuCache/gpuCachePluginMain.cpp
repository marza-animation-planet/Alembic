...
MStatus initializePlugin( MObject obj )
{
    ...
    status = plugin.registerUIStrings(registerMStringResources, "altGpuCacheInitStrings");
    if (!status)
    {
    ...
    status = plugin.registerCommand(
        "altGpuCache", Command::creator, Command::cmdSyntax );
    if (!status) {
    ...
    if (MGlobal::kInteractive == MGlobal::mayaState()) {
        ...
        MString registerCustomFilterCMD = "addCustomOutlinerFilter(\"altGpuCache\",\"CustomAltGPUCacheFilter\",\"";
    ...
    MGlobal::executeCommand("setNodeTypeFlag -threadSafe true altGpuCache");

    MGlobal::executeCommandOnIdle("altGpuCacheCreateUI");
...
MStatus uninitializePlugin( MObject obj )
{
    ...
    if (MGlobal::kInteractive == MGlobal::mayaState()) {
        MString unregisterCustomFilterCMD = "deleteCustomOutlinerFilter(\"CustomAltGPUCacheFilter\")";
    ...
    status = plugin.deregisterCommand( "altGpuCache" );
    if (!status) {
        status.perror("deregisterCommand");
        return status;
    }

    MGlobal::executeCommandOnIdle("altGpuCacheDeleteUI");
