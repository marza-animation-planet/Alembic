...
{
public:
    BoundingBoxVisitor(double timeInSeconds, bool NTR, bool ignoreVis)
      : fTimeInSeconds(timeInSeconds)
      , fNTR(NTR)
      , fIgnoreVis(ignoreVis)
    {}
                       ...
                       const SubNode&     subNode)
    {
        ...
        if (sample) {
            // fBoundingBox = (fNTR ? sample->boundingBoxNTR() : sample->boundingBox());
            fBoundingBox.clear();
            if (fIgnoreVis || sample->visibility()) {
                BOOST_FOREACH (const SubNode::Ptr& child, subNode.getChildren()) {
                    BoundingBoxVisitor childVis(fTimeInSeconds, fNTR, fIgnoreVis);
                    child->accept(childVis);
                    fBoundingBox.expand(childVis.boundingBox());
                }
            }
        }
    }
    ...
    {
        if (subNode) {
            BoundingBoxVisitor visitor(timeInSeconds, NTR, ignoreVis);
...
inline bool ValidateGeomPath(const SubNode::Ptr& top, const MString& geomPath, MString& validatedGeomPath)
{
    ...
    for( unsigned int i = 0; i < pathArray.length(); i++ )
	{
        ...
        // Note: for a single root node, top is not "|" but the name of the root node
        if (i == 0 && top->getName() != "|")
        {
            foundChild = (step == top->getName());
        }
		else
        {
