...
void EchoBounds(const MString &header, const MBoundingBox &bbox)
{
    char buffer[2048];
    sprintf(buffer, "%s: [%f, %f, %f] - [%f, %f, %f]", header.asChar(),
...
DataProvider::getBBoxPlaceHolderSample(double seconds, const MMatrix &rootMatrix)
{
    MBoundingBox bboxNTR = getBoundingBox();
    MBoundingBox bbox = bboxNTR;
    bbox.transformUsing(rootMatrix);
...
boost::shared_ptr<const ShapeSample> 
RawDataProvider::getSample(double seconds, const MMatrix &rootMatrix)
{
...
boost::shared_ptr<const ShapeSample> 
Triangulator::getSample(double seconds, const MMatrix &rootMatrix)
{
    // empty mesh
...
boost::shared_ptr<const ShapeSample> 
NurbsTessellator::getSample(double seconds, const MMatrix &rootMatrix)
{
    // empty mesh
    if (!fWireIndices || !fTriangleIndices) {
        boost::shared_ptr<ShapeSample> sample =
...
boost::shared_ptr<const ShapeSample> 
SubDSmoother::getSample(double seconds, const MMatrix &rootMatrix)
{
    // empty mesh
    if (!fWireIndices || !fTriangleIndices) {
        boost::shared_ptr<ShapeSample> sample =
    ...
    const MMatrix& rootMatrix, TimeInterval rootMatrixInterval)
{
    ...
    BOOST_FOREACH(const AlembicCacheObjectReader::Ptr& childReader, fChildren) {
        ...
        bboxNTR.expand(childReader->getBoundingBoxNTR());
...
MBoundingBox AlembicCacheTopReader::getBoundingBoxNTR() const
{
    return fBoundingBoxNTR;
}
      ...
      fBoundingBoxValidityInterval(TimeInterval::kInvalid)
{
    ...
    if (numSamples > 0) {
        animTimeRange |= TimeInterval(timeSampling->getSampleTime(0),
                                      timeSampling->getSampleTime(numSamples > 0 ? numSamples-1 : 0));
    }
...
MBoundingBox AlembicCacheXformReader::getBoundingBoxNTR() const
{
    return fBoundingBoxNTR;
}
    ...
    const MMatrix& rootMatrix, TimeInterval rootMatrixInterval)
{
    ...
    fBoundingBoxNTR = fDataProvider->getBoundingBox();
    fBoundingBox = fBoundingBoxNTR;
    ...
    if (seconds == validityInterval.startTime()) {
        boost::shared_ptr<const ShapeSample> sample = 
            fDataProvider->getBBoxPlaceHolderSample(seconds, rootMatrix);
...
TimeInterval AlembicCacheMeshReader::sampleShape(double seconds)
{
    ...
    // interval.
    if (seconds == validityInterval.startTime()) {
        // In order to be able to dynamically modify visibility, always read shape data
        // if (fDataProvider->isVisible()) {
            MMatrix rootMatrix = MMatrix::identity;
            std::map<double, MMatrix>::const_iterator it = fMatrices.find(seconds);
            if (it != fMatrices.end()) {
                rootMatrix = it->second;
            }
            boost::shared_ptr<const ShapeSample> sample = 
                fDataProvider->getSample(seconds, rootMatrix);
            fShapeData->addSample(sample);
...
MBoundingBox AlembicCacheMeshReader::getBoundingBoxNTR() const
{
    return fBoundingBoxNTR;
}
...
void AlembicCacheMeshReader::saveAndReset(AlembicCacheReader& cacheReader)
{
    ...
    fBoundingBox.clear();
    fBoundingBoxNTR.clear();
    // Don't clear matrices, shapes are read in bulk after hierarchy
    ...
    const MString& geomPath, bool needUVs)
{
    ...
    try {
        ...
        if (pathArray.length() == 0) {
            ...
            //         current, needUVs);
            // }
            reader = boost::make_shared<AlembicCacheTopReader>(current, needUVs);
        }
        else {
