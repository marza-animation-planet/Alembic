...
using namespace GPUCache;
    ...
    MColor getTexture2dDefaultColor(const MPlug& plug)
    {
        ...
        if (plug.connectedTo(connections, true, false)) {
            ...
            float r, g, b;
            statusR = diffusePlugR.getValue(r);
            statusG = diffusePlugG.getValue(g);
            statusB = diffusePlugB.getValue(b);
    ...
    private:
            ...
            const boost::shared_ptr<const ShapeSample>& shape)
        {
            ...
            if (xformMatrix.isEquivalent(MMatrix::identity)) {
                ...
                boundingBoxNTR = shape->boundingBoxNTR();
            }
            else {
                float xform[4][4];
                float xformIT[4][4];
                ...
                for (size_t i=0; i<numVerts; ++i) {
                    ...
                    lMinX = std::min(x, lMinX);
                    lMinY = std::min(y, lMinY);
                    lMinZ = std::min(z, lMinZ);

                    lMaxX = std::max(x, lMaxX);
                    lMaxY = std::max(y, lMaxY);
                    lMaxZ = std::max(z, lMaxZ);
                ...
                boundingBoxNTR = MBoundingBox(MPoint(lMinX, lMinY, lMinZ),
                    MPoint(lMaxX, lMaxY, lMaxZ));
            }
...

            {
...

                {
                    ...
                    for (size_t i = 0; i < nbShapes; i++) {
                        ...
                        boundingBoxNTR.expand(sample->boundingBoxNTR());
                    }
                }
    ...
    public:
        ...
        void promptOverwrite()
        {
            ...
            MGlobal::executeCommand(
                "if (!(`exists showAltGpuCacheExportConfirmDialog`))\n"
                "{\n"
                "    eval(\"source \\\"doAltGpuCacheExportArgList.mel\\\"\");\n"
                "}\n"
            );
            ...
            BOOST_FOREACH (RootMapType::value_type& v, fRootMap) {
                ...
                if (choice == kUnknownOverwrite) {
                    ...
                    MString result = MGlobal::executeCommandStringResult(
                        "showAltGpuCacheExportConfirmDialog \"" + 
