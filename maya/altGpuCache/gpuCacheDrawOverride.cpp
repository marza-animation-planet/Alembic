...
public:
    ...
    TopLevelCullVisitor(const Frustum& frustrum, double seconds, bool ignoreTr)
        : fFrustum(frustrum),
          fSeconds(seconds),
          fIsCulled(true),
          fIgnoreTransforms(ignoreTr)
    {}
                       ...
                       const SubNode&     subNode)
    {
        ...
        fIsCulled =
            fFrustum.test(fIgnoreTransforms ? sample->boundingBoxNTR() : sample->boundingBox()) == Frustum::kOutside;
    }
...
public:
    ...
    void set(const double& seconds,
             ...
             bool          isSelected)
    {
        fSeconds = fShapeNode->getTime(seconds, &fClip);
    ...
    const ShapeNode* getShapeNode() const
    {
        return fShapeNode;
    }
...
{
    DrawShadedState state(
        frustum, fSeconds, transparentPrune,
        fShapeNode->ignoreTransforms(),
        fShapeNode->ignoreVisibility(),
        stateMgr, cullMode, depthOffsetType, colorType,
    ...
    const SubNode::Ptr& rootNode) const
{
    ...
    if (xformData) {
        ...
        if (!sample || (!fShapeNode->ignoreVisibility() && !sample->visibility())) return;
        boundingBox = (fShapeNode->ignoreTransforms() ? sample->boundingBoxNTR() : sample->boundingBox());
    }
    else {
    ...
    const MHWRender::MDrawContext& context) const
{
    ...
    {
        TopLevelCullVisitor visitor(frustum, fSeconds, fShapeNode->ignoreTransforms());
        rootNode->accept(visitor);
    ...
    const MDagPath& cameraPath) const
{
    ...
    bool clip = false;
    const double seconds = shapeNode->getTime(&clip);
    if (clip) {
        return MBoundingBox();
    }
    ...
    return MBoundingBox();
    */
    return BoundingBoxVisitor::boundingBox(subNode, seconds, shapeNode->ignoreTransforms(), shapeNode->ignoreVisibility());
}
