...
{
public:
    SubNodeUserDataDelegate(const boost::shared_ptr<SubNodeUserData>& userData)
        : MUserData(true /*deleteAfterUse*/),
          fUserData(userData)
    {}

    virtual ~SubNodeUserDataDelegate()
    {}
...
private:
    boost::shared_ptr<SubNodeUserData>  fUserData;
};
...
public:
    ...
    void setCustomData(const boost::shared_ptr<SubNodeUserData>& userData)
    {
    ...
    const boost::shared_ptr<SubNodeUserData>& userData() const  { return fUserData; }
...
public:
                           ...
                           const ShapeData&    shape)
    {
        ...
        if (fBoundingBoxItem) {
            const boost::shared_ptr<const ShapeSample>& sample =
                shape.getSample(subSceneOverride.getTime());
            if (sample) {
                const MBoundingBox& boundingBox = (subSceneOverride.getShapeNode()->ignoreTransforms() ? sample->boundingBoxNTR() : sample->boundingBox());
                       ...
                       const ShapeData&    shape)
    {
        ...
        // Streams are all in object space, matrix is set separately
        MBoundingBox bbox = sample->boundingBoxNTR();
                                ...
                                const SubNode&      subNode)
    {
        ...
        if (!fBoundingBoxItem) {
            ...
            fBoundingBoxItem->setCustomData(
                boost::make_shared<SubNodeUserData>(subNode)
            );
        }
                       ...
                       const SubNode&     subNode)
    {
        ...
        if (hierarchyStat) {
            ...
            if (!fDontPrune) {
                ...
                if (!fTraverseInvisible && !fSubSceneOverride.getShapeNode()->ignoreVisibility()) {
                    const boost::shared_ptr<const XformSample>& sample =
...
public:
    ...
    bool canPrune(const HierarchyStat::SubNodeStat& stat)
    {
        return fSubSceneOverride.getShapeNode()->ignoreVisibility() || !stat.isVisibilityAnimated;
    }
                ...
                SubNodeRenderItems::Ptr& subNodeItems)
    {
        ...
        // NOTE: do not take into account shape visibility
        //       doesn't provide expected results for instances
                       ...
                       const SubNode&     subNode)
    {
        ...
        if (!fSubSceneOverride.getShapeNode()->ignoreVisibility()) {
...
public:
    ...
    bool canPrune(const HierarchyStat::SubNodeStat& stat)
    {
        return fSubSceneOverride.getShapeNode()->ignoreTransforms() || !stat.isXformAnimated;
    }
                       ...
                       const SubNode&     subNode)
    {
        ...
        if (!fSubSceneOverride.getShapeNode()->ignoreTransforms()) {
                           ...
                           MSubSceneContainer& container)
    {
        ...
        if (fBoundingBoxItem) {
            const MBoundingBox boundingBox = BoundingBoxVisitor::boundingBox(
                ...
                subSceneOverride.getShapeNode()->ignoreTransforms(),
                subSceneOverride.getShapeNode()->ignoreVisibility()
            );
            const MMatrix worldMatrix =
                                      ...
                                      const MFrameContext&      frameContext) const
{
    ...
    if (geometry && frameContext.getLightingMode() == MFrameContext::kLightDefault) {
        ...
        BoundingBoxVisitor visitor(abcTime, fShapeNode->ignoreTransforms(), fShapeNode->ignoreVisibility());
                              ...
                              const MFrameContext& frameContext)
{
    ...
    bool forceUpdate = (fShapeNode->timeControlChanged() || fShapeNode->transformsChanged() || fShapeNode->visibilityChanged());
    fShapeNode->acknowledgeChanges();
    ...
    bool clip = false;
    fTimeInSeconds = fShapeNode->getTime(&clip);
    if (forceUpdate || clip != fClipped) {
        fClipped = clip;
        // Rebuild render items.
        fInstanceRenderItems.clear();
        container.clear();
