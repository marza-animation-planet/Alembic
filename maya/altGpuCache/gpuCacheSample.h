...
public:
    ...
    static boost::shared_ptr<ShapeSample> create(
        ...
        const MBoundingBox& boundingBoxNTR,
    ...
    static boost::shared_ptr<ShapeSample> createBoundingBoxPlaceHolderSample(
        double timeInSeconds, const MBoundingBox& bbox, const MBoundingBox& bboxNTR, bool visibility)
    {
    ...
    { return fBoundingBox; }
    const MBoundingBox& boundingBoxNTR() const
...
private:
    ...
    const MBoundingBox                                  fBoundingBoxNTR;
...
public:
	...
	static boost::shared_ptr<XformSample> create(
		...
		const MMatrix&      xform,
		const MBoundingBox& boundingBox,
        const MBoundingBox& boundingBoxNTR, // bounding box without transforms
		bool                visibility)
	{
		return boost::make_shared<XformSample>(
			timeInSeconds, xform, boundingBox, boundingBoxNTR, visibility);
	}
    ...
    const MBoundingBox& boundingBoxNTR() const { return fBoundingBoxNTR; }
	bool visibility() const                 { return fVisibility; }
	...
	XformSample(double                 timeInSeconds,
        ...
        : fTimeInSeconds(timeInSeconds),
          ...
          fBoundingBoxNTR(boundingBoxNTR),
