...
public:
    ...
    DrawTraversalState(
        ...
        : fFrustum(frustrum),
          ...
          fIgnoreTransforms(ignoreTransforms),
          fIgnoreVisibility(ignoreVisibility)
    {}
	virtual ~DrawTraversalState() {}

    const Frustum&  frustum() const { return fFrustum; }
    double          seconds() const { return fSeconds; }
    bool   ignoreTransforms() const { return fIgnoreTransforms; }
    bool   ignoreVisibility() const { return fIgnoreVisibility; }
...
private:
    ...
    VBOProxy                   fVBOProxy;
    const bool                 fIgnoreTransforms;
    const bool                 fIgnoreVisibility;
};
...
private:
                       ...
                       const SubNode&     subNode)
    {
        ...
        if (!state().ignoreVisibility() && !sample->visibility()) return;
        ...
        if (fParentClippingResult != Frustum::kInside) {
            ...
            clippingResult = state().frustum().test(
                    (state().ignoreTransforms() ? sample->boundingBoxNTR() : sample->boundingBox()), fParentClippingResult);
        ...
        MMatrix matrix = (state().ignoreTransforms() ? fXform : (sample->xform() * fXform)); 
        Derived traversal(fState, matrix,
