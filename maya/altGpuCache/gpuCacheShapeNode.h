...
public:
    ...
    static MObject aIgnoreTransforms;
   ...
   inline bool timeControlChanged() const { return fTimeControlChanged; }
   inline bool transformsChanged() const { return fTransformsChanged; }
   inline bool visibilityChanged() const { return fVisibilityChanged; }
   inline void acknowledgeChanges() const {
      fTimeControlChanged = false;
      fTransformsChanged = false;
      fVisibilityChanged = false;
   }
   inline bool ignoreTransforms() const { return fIgnoreTransforms; }
   inline bool ignoreVisibility() const { return fIgnoreVisibility; }
   double getTime(bool *clip=0) const;
   double getTime(double inTime, bool *clip=0) const;
...
private:
   ...
   double getFPS() const;
   double timeWarp(const double inputTime, const double firstTime, const double lastTime, CycleType cycleType, bool *clip) const;


    mutable MString fCacheFileName;
	mutable MString fResolvedCacheFileName;
    mutable MString fCacheGeomPath;
    mutable bool fIgnoreTransforms;
    mutable bool fIgnoreVisibility;
    mutable TimeControl fTimeCtrl;
    mutable bool fTimeControlChanged;
    mutable bool fTransformsChanged;
    mutable bool fVisibilityChanged;
