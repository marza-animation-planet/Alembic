...
template <>
void _NodeSet<AI_TYPE_BOOLEAN, uint8_t>(AtNode *node, const char *name, uint8_t *vals)
{
...
{
   AiNodeSetBool(node, name, (uint8_t)vals[0]);
}
...
template <>
void _NodeSet<AI_TYPE_INT, uint8_t>(AtNode *node, const char *name, uint8_t *vals)
{
...
template <>
void _NodeSet<AI_TYPE_UINT, uint8_t>(AtNode *node, const char *name, uint8_t *vals)
{
...
template <>
void _NodeSet<AI_TYPE_FLOAT, uint8_t>(AtNode *node, const char *name, uint8_t *vals)
{
...
template <>
void _ArraySet<AI_TYPE_BOOLEAN, uint8_t>(AtArray *ary, unsigned int count, uint8_t *vals, unsigned int *idxs)
{
   if (idxs)
   {
      for (unsigned int i=0; i<count; ++i)
...
void _ArraySet<AI_TYPE_BYTE, int>(AtArray *ary, unsigned int count, int *vals, unsigned int *idxs)
{
   if (idxs)
   {
      for (unsigned int i=0; i<count; ++i)
      {
         AiArraySetByte(ary, i, (uint8_t)vals[idxs[i]]);
      }
   }
   else
   {
      for (unsigned int i=0; i<count; ++i)
      {
         AiArraySetByte(ary, i, (uint8_t)vals[i]);
      }
   }
}
...
template <>
void _ArraySet<AI_TYPE_INT, uint8_t>(AtArray *ary, unsigned int count, uint8_t *vals, unsigned int *idxs)
{
   if (idxs)
   {
      for (unsigned int i=0; i<count; ++i)
...
template <>
void _ArraySet<AI_TYPE_UINT, uint8_t>(AtArray *ary, unsigned int count, uint8_t *vals, unsigned int *idxs)
{
   if (idxs)
   {
      for (unsigned int i=0; i<count; ++i)
...
template <>
void _ArraySet<AI_TYPE_FLOAT, uint8_t>(AtArray *ary, unsigned int count, uint8_t *vals, unsigned int *idxs)
{
   if (idxs)
   {
      for (unsigned int i=0; i<count; ++i)
...
void _SetUserAttribute(AtNode *node, const std::string &valName, const std::string &idxName, UserAttribute &ua, unsigned int *remapIndices)
{
   ...
   case AI_TYPE_BOOLEAN:
      ...
      break;
   case AI_TYPE_BYTE:
      __SetUserAttribute<ArnoldType, uint8_t>(node, valName, idxName, ua, remapIndices);
      break;
   case AI_TYPE_INT:
...
void SetUserAttribute(AtNode *node, const char *name, UserAttribute &ua, unsigned int count, unsigned int *remapIndices)
{
   ...
   case AI_TYPE_FLOAT:
      ...
      break;
   case AI_TYPE_VECTOR2:
      _SetUserAttribute<AI_TYPE_VECTOR2>(node, valsName, idxsName, ua, remapIndices);
      break;
   case AI_TYPE_VECTOR:
